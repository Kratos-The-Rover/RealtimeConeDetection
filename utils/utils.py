...
def plot_one_box(x, img, line_thickness=None):  # Plots one bounding box on image img
    ...
    # col1 = img[int(0.75 * c1[1] + 0.25 * c2[1]), int(0.5 * (c1[0] + c2[0]))]
    # col2 = img[int(0.5 * c1[1] + 0.5 * c2[1]), int(0.5 * (c1[0] + c2[0]))]
    # col3 = img[int(0.25 * c1[1] + 0.75 * c2[1]), int(0.5 * (c1[0] + c2[0]))]
    # That's extracting the central 3 points, may be faster

    # pay special attention: in opencv, image's shape:[h,w,d].
    # When extracting one point:img[h,w], but when draw circle, rec or so, draw((w,h))
    # And color is in BGR format
